...
`;
  ...
  const migrations = [
    ...
    // Finance module — account currency plus per-reservation money in
    // minor units (cents). All nullable/defaulted so existing rows are
    // untouched.
    ...
    `CREATE TABLE IF NOT EXISTS "Expense" (
        ...
        CONSTRAINT "Expense_reservationId_fkey" FOREIGN KEY ("reservationId") REFERENCES "Reservation" ("id") ON DELETE SET NULL ON UPDATE CASCADE
    )`,
    `CREATE INDEX IF NOT EXISTS "Expense_propertyId_date_idx" ON "Expense"("propertyId", "date")`,
    `CREATE INDEX IF NOT EXISTS "Expense_recurGroup_idx" ON "Expense"("recurGroup")`,
    // Smart-lock module — provider credentials (secret encrypted with
    // the guest-data key), per-property device binding, and per-
    // reservation door codes.
    `CREATE TABLE IF NOT EXISTS "LockCredential" (
        "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
        ...
        "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "updatedAt" DATETIME,
        CONSTRAINT "LockCredential_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User" ("id") ON DELETE CASCADE ON UPDATE CASCADE
    )`,
    `CREATE UNIQUE INDEX IF NOT EXISTS "LockCredential_userId_key" ON "LockCredential"("userId")`,
    `CREATE TABLE IF NOT EXISTS "AccessCode" (
        "id" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
        "reservationId" INTEGER NOT NULL,
        "propertyId" INTEGER NOT NULL,
        "provider" TEXT NOT NULL DEFAULT 'tuya',
        "remotePasswordId" TEXT,
        ...
        "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        "updatedAt" DATETIME,
        CONSTRAINT "AccessCode_reservationId_fkey" FOREIGN KEY ("reservationId") REFERENCES "Reservation" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
        CONSTRAINT "AccessCode_propertyId_fkey" FOREIGN KEY ("propertyId") REFERENCES "Property" ("id") ON DELETE CASCADE ON UPDATE CASCADE
    )`,
    `CREATE UNIQUE INDEX IF NOT EXISTS "AccessCode_reservationId_key" ON "AccessCode"("reservationId")`,
    `CREATE INDEX IF NOT EXISTS "AccessCode_propertyId_idx" ON "AccessCode"("propertyId")`,
    `ALTER TABLE "Property" ADD COLUMN "lockDeviceId" TEXT`,
    `ALTER TABLE "Property" ADD COLUMN "lockDeviceName" TEXT`,
  ];
