...
function AppContent({
  ...
  const handleUpdateProperty = async (id: number, data: { name?: string; minNights?: number; checkInTime?: string; checkOutTime?: string; bookingWindow?: number; lockDeviceId?: string | null; lockDeviceName?: string | null }) => {
  ...
  const renderContent = () => {
    ...
    // Calendar is dual-mode too: with no property selected, show every
    // property as one row on a shared date axis so occupancy can be
    // compared at a glance; picking a property (via the switcher, the
    // top-bar dropdown, or clicking a row) drops into the full
    // single-property calendar below.
    if (activeView === "calendar" && !selectedProperty) {
      return <GlobalCalendarView properties={properties} />;
    }
        ...
        case "sync":
          return (
            <SyncSettings
              ...
              lockDeviceId={selectedProperty.lockDeviceId}
              lockDeviceName={selectedProperty.lockDeviceName}
