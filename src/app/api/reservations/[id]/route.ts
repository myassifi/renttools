...
export async function PATCH(
  ...
  try {
    ...
    // Money fields — minor units (cents) of the account currency.
    // null/"" clears a field; any other value must be a non-negative
    // integer (the UI converts the decimal input before sending).
    for (const key of [
      "grossCents",
      "hostFeeCents",
      "cleaningFeeCents",
