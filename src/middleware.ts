...
export async function middleware(request: NextRequest) {
  ...
  // Requests that already went through the locale-prefix rewrite carry
  // our x-locale header. Next re-runs middleware on the internal
  // rewrite target, so without this guard /fr → rewrite / → cookie
  // redirect /fr → infinite self-redirect (ERR_TOO_MANY_REDIRECTS).
  const alreadyLocalized = request.headers.has("x-locale");
