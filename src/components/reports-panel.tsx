...
import { reservationDayKey } from "@/lib/reservation-dates";
...
export function buildStaysForProperty(
  ...
  for (const r of prop.reservations) {
    ...
    reservationDateKeys.add(
      `${normalizedPlatform(r.platform)}\u0000${reservationDayKey(r.checkIn)}|${reservationDayKey(r.checkOut)}`,
    );
  }
  ...
  for (const r of prop.reservations) {
    stays.push({
      start: reservationDayKey(r.checkIn),
      end: reservationDayKey(r.checkOut),
      platform:
