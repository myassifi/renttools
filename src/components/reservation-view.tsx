...
import { ReservationMoney } from "@/components/reservation-money";
import { ReservationAccess } from "@/components/reservation-access";
...
export function ReservationView({
  ...
  return (
    ...
    <div className="mx-auto max-w-[1760px] space-y-6 px-3 sm:px-5">
      ...
      {/* Money — gross / platform fee / cleaning fee / payout for this
          booking. Feeds the Income & expenses section in Reports. */}
      <ReservationMoney
        reservation={reservation}
        propertyName={propertyName}
        guestName={resolveLeadGuestFirstName(reservation, guests)}
        onUpdate={onUpdateReservation}
      />

      {/* Door access — temporary PIN on the property's smart lock, valid
          for the stay window. Hidden unless a lock device is bound. */}
      <ReservationAccess
        reservationId={reservation.id}
        hasLock={!!lockDeviceId}
        lockName={lockDeviceName}
        onCodeChange={setDoorCode}
      />
        ...
        <div className="space-y-1 border-t border-border/30 pt-2">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <a
              href={
                reservationPhoneEnabled
                  ? `sms:${reservationPhoneForLinks}?&body=${encodeURIComponent(reservationMessengerPrefill)}`
                  : undefined
              }
              aria-disabled={!reservationPhoneEnabled}
              tabIndex={reservationPhoneEnabled ? 0 : -1}
              ...
              title={tr("guest.messageOnSms")}
              className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md transition-all ${
                reservationPhoneEnabled
                  ? "bg-[var(--m-accent)]/15 text-[var(--m-accent)] hover:bg-[var(--m-accent)]/25"
                  : "bg-white/5 text-muted-foreground/30 cursor-not-allowed"
              }`}
            >
