...
export function TopBar({
  ...
  // Property-required views — Sync settings only make sense in a
  // property context. If user clicks it with no property selected,
  // auto-pick the first one in the same nav.
  const goToTab = (view: AppView) => {
    ...
    // Sync is the only tab that strictly requires a property — each
    // property has its own calendar links. Calendar, Cleaning and
    // Reports are dual-mode: routing them with no property lands on
    // the global aggregate view, not a forced first-property pick.
    const requiresProperty = view === "sync";
